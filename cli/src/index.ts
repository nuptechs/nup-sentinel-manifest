...
program
  .name('permacat')
  .description('PermaCat CLI - Code-to-Permission Catalog Generator')
  .version('0.1.0')
  .option('--server <url>', 'PermaCat server URL', 'http://localhost:5000')
  .option('--key <apiKey>', 'API key for authentication')
  .option('--config <path>', 'Path to config file', '~/.permacat.json');
